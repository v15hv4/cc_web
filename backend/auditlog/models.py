        ...
        if changes is not None:
            ...
            if kwargs.get("action", None) is LogEntry.Action.CREATE:
                if (
                    ...
                    and self.filter(
                        content_type=kwargs.get("content_type"), object_id=kwargs.get("object_id")
                    ).exists()
                ):
                    self.filter(
                        content_type=kwargs.get("content_type"), object_id=kwargs.get("object_id")
            ...
            return (
                self.create(**kwargs) if db is None or db == "" else self.using(db).create(**kwargs)
            )
        return None
    ...
    def changes_str(self, colon=": ", arrow=smart_text(" \u2192 "), separator="; "):
        ...
        for field, values in iteritems(self.changes_dict):
            substring = smart_text("{field_name:s}{colon:s}{old:s}{arrow:s}{new:s}").format(
                field_name=field, colon=colon, old=values[0], arrow=arrow, new=values[1],
            )
